...
| `image` | Yes | `"nvcr.io/..."` | Resolve the default with `scripts/resolve_tao_image.py --model <network_arch> --action <action>`, show it to the user, and require confirmation or `image=<override>` before creating the AutoML runner. |
...
- `${TAO_SKILL_BANK_PATH:-~/tao-skills-external}/models/<network>/schemas/<action>.schema.json`
...
AutoML runs the selected TAO action. Before generating any AutoML script, read `<bank-root>/models/<network>/SKILL.md` (where `<bank-root>` is wherever the agent loaded this SKILL.md from). The model skill contains all model-specific knowledge:
