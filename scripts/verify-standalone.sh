...
echo
echo "=== 3. Read visual-changenet metadata from the skill bank"
INFO_FILE=""
for f in models/visual-changenet/references/skill_info.yaml \
         models/visual-changenet/references/model_info.yaml; do
  [ -f "$f" ] && INFO_FILE="$f" && break
done

if [ -z "$INFO_FILE" ]; then
  echo "  visual-changenet has no references/skill_info.yaml or references/model_info.yaml"
...
echo "ask the agent to run a real visual-changenet inference. It should construct"
echo "docker run from SKILL.md (and references/ if present), invoke via Bash, and"
echo "produce output — no Python imports required."
