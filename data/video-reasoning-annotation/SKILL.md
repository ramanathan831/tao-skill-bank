...
If the user has **no endpoint at all** and wants to self-host, point them at the `applications/tao-inference` skill — a workflow that stands up a network-specific TAO inference microservice locally and exposes an OpenAI-compatible endpoint. Should support Cosmos, Qwen, and Gemma. Check `applications/tao-inference/references/service.yaml` for the current `valid_network_arch_config_basenames` list before relying on a specific model.
