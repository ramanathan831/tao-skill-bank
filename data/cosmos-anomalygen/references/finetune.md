...
Detail for `cosmos-anomalygen` Phases 0 and 1. Read before executing either phase.
...
mask counts printed by `validate_dataset.py`). This produces n_seeds=1 so
every training submask appears exactly once. The script emits every defect
type from `defect_spec` — any type with zero masks will cause
`allocate_samples.py` to error and must be fixed before continuing.

**KPI floor:** every defect type must receive ≥ 3 JSONL entries for `nn_score`
to be meaningful. With proportional allocation this is naturally satisfied when
total mask count ≥ 3 × number of defect types. If any type has very few masks
(e.g. 1–2), increase `num_SDG` to `ceil(3 × N_total / N_min)` where `N_min`
is the smallest per-type mask count.
