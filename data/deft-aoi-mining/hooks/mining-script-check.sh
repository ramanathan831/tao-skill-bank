...
# 1b. tao-toolkit-ds:aoi image missing or unreachable
if re.search(r'(unable to find image|pull access denied|manifest unknown|repository does not exist).*tao-toolkit-ds:aoi', combined, re.IGNORECASE):
    warnings.append("Container image `nvcr.io/nvidian/iva/tao-toolkit-ds:aoi` is missing or unreachable. Pre-pull with `docker pull nvcr.io/nvidian/iva/tao-toolkit-ds:aoi` and confirm registry credentials. The `:aoi` tag is required — the generic `:latest` does not contain the embedding/mining entrypoints.")
