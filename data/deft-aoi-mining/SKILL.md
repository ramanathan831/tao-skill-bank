...
> **Known incompatibility (6.26.6-rc and later):** the `-e <spec>` flag was removed and the `experiment_specs/` config directory shipped with earlier images is missing. Running `embedding image_embeddings -e <spec>` against `tao_toolkit.data_services:6.26.6-rc-175-multiarch` (or newer) raises `Primary config directory not found`. The replacement pattern is the same as for `gap_analysis vcn_aoi` (see `data/deft-aoi-rca-vcn/SKILL.md` → "Note (6.26.6-rc and later)"): drop `-e <spec>`, pass every value as a Hydra `key=value` override. Schema keys may also be renamed (the RCA skill saw `inference_csv` → `inference_results_dir`, `output_dir` → `results_dir`); introspect the actual schema with `docker run ... embedding image_embeddings --cfg=job` and `... tmm nearest_neighbors --cfg=job` before invoking, since key names diverge per release.
