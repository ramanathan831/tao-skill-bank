"""
Side-flip defect — run directly in Isaac Sim Script Editor.
Component flips sideways, rotating around bottom edge Y-axis.
...
if not prim.IsValid():
    ...
    if not prim.HasAttribute("semantics:labels:defect"):
        rep_modify.semantics(prim, value={"defect": "sideflip"})
        print(f"[Info] Applied semantic label: defect=sideflip")
    else:
    ...
    # Step 3: Apply sideflip transform (Fabric layer)
