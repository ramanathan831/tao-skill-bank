...
- `tao-skill-bank:brev` — running docker via `brev exec` on a Brev instance
- `tao-skill-bank:lepton` — submitting jobs to Lepton (API-first; `docker run` doesn't apply)
- `tao-skill-bank:tao-sdk` — optional Python layer wrapping docker invocations with Job handles, state persistence, and S3 I/O
