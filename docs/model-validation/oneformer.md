...
- deploy gen_trt_engine: pass in rebuilt deploy image `nvcr.io/nvstaging/tao/tao-toolkit-deploy:validation-fixes-20260525`
- deploy evaluate: pass in rebuilt deploy image
- deploy inference: pass in rebuilt deploy image
...
- Source-fixed deploy rerun checkpoint used: resolver-selected exact checkpoint `/tmp/tao-source-fixed-rerun/current/oneformer/results/train/model_epoch_000_step_00002.pth`
- Source-fixed export/deploy handoff: export wrote `/workspace/results/export/oneformer_640.onnx`; deploy generated `/workspace/results/deploy_gen/oneformer_640.engine`
...
- Config issues:
  - Tiny export shapes can trigger a PyTorch ONNX shape-inference failure. The source-fixed rerun reproduced that at 320x320, then passed export at the default 640x640 shape.
- Dataset issues:
...
- Docker/local execution issues:
  ...
  - The rebuilt deploy image parsed both ONNX inputs correctly: `images` as `(1, 3, 640, 640)` and `task_tokens` as `(1, 77)`.
...
- Reran OneFormer deploy with `nvcr.io/nvstaging/tao/tao-toolkit-deploy:validation-fixes-20260525`; `gen_trt_engine`, TensorRT inference, and TensorRT evaluate all passed.

## Remaining issues

- The original default deploy image still contains the two-input ONNX parsing bug. The source-fixed rebuilt deploy image validates the OneFormer TensorRT path.
...
- Fully validated for packaged model-skill actions after the source-fixed deploy rerun; deploy TensorRT actions require the rebuilt deploy image until that source fix is promoted to the default image.
