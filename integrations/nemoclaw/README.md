...
| `tao_run` | Launch a container on the host GPU (constrained: `nvcr.io/*` NGC images only, workspace-confined per-job results, host UID:GID ownership, `shm_size` for DataLoaders) |
