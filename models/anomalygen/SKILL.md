---
name: anomalygen
description: "Cosmos AnomalyGen SDG pipeline: synthetic defect image generation for AOI. Use when generating synthetic anomaly images for PCB defect detection training data augmentation."
---
...
```
python -m scripts.automatic_mask_placement \
  --clean_image_dir /data/clean_images/{defect_type}/ \
  --roi_dir /data/roi/{defect_type}/ \
  --submask_dir /data/submasks/{defect_type}/ \
  --output_dir /data/amp_output/{defect_type}/ \
  --N 5
```

Run per defect type. Only process defect types that have BOTH ROI and submask directories.

### Step 2: Build sdg_inference.jsonl

Generate one JSONL entry per AMP output mask. For each entry, randomize parameters:

```python
import json, random

entries = []
for amp_mask_path in amp_masks:
    entry = {
        "image_filename": clean_image_path,
        "mask_filename": amp_mask_path,
        "anomaly_type": f"PCB+{defect_type}",
        "guidance": random.choice([1.5, 5.0, 7.0]),
        "num_steps": 35,
        "crop_and_paste": True,
        "crop_ratio": 2.0,
        "num_generated_images": 1,
    }
    # Spatial dependency from defect description
    if spatial_dependency == "position_dependent":
        entry.update({"shift_values": "0,0", "rotation_angle": 0, "morph_operation": "none"})
    else:  # "free"
        entry.update({
            "shift_values": f"{random.randint(-100,100)},{random.randint(-100,100)}",
            "rotation_angle": random.randint(0, 180),
            "morph_operation": random.choice(["none", "dilate", "erode"]),
        })
    entries.append(json.dumps(entry))
```

### Step 3: SDG Inference via torchrun

```
torchrun --nproc_per_node 1 -m scripts.sdg_inference \
  -- experiment=predict2_anomaly_gen_fsdp_2b \
  inference.sdg_inference_file=/data/sdg_inference.jsonl \
  inference.output_dir=/data/output/ \
  model.anomalygen.checkpoint_path=/checkpoints/anomalygen/step_{STEP}
```

The `--` before `experiment=` is REQUIRED — it separates torchrun args from config overrides.

## Pretrained Models

The container requires three models mounted at `checkpoints/`. On first run, execute the built-in download script inside the container:

```
python -m scripts.download_checkpoints --model_types text2image --model_sizes 2B
```

This is idempotent and downloads: Cosmos Predict2 2B (gated — needs HF_TOKEN), NV-DINOv2, C-RADIOv3.
