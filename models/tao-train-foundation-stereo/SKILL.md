...
DepthNet Stereo training writes checkpoint files under the Lightning task subdirectory for the train action when using the packaged spec layout (for example, top-level `results_dir: /results/train` with `train.results_dir: ''` writes `/results/train/train/model_epoch_<epoch>_step_<step>.pth`) and creates a `dn_model_latest.pth` symlink in the same directory. For `evaluate`, `inference`, `export`, `quantize`, and resume/retrain, select checkpoints through the SDK/model resolver so a requested best, epoch, or step checkpoint resolves to that exact file. Use `dn_model_latest.pth` only when the user explicitly asks for latest.
