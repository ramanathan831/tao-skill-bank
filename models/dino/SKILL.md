...
name: dino
description: >-
  DINO (DETR with Improved DeNoising Anchor Boxes) for 2D object detection. Use when the user asks to
  train, evaluate, export, quantize, distill, generate TensorRT engines, or run inference for TAO DINO
  object detectors, COCO datasets, or DINO AutoML.
license: Apache-2.0
metadata:
...
Current runner limitation: DINO data sources must be supplied explicitly in
`spec_overrides` because `_apply_data_sources()` does not auto-resolve the
TAO Core `mapping` sub-structure for this model. Do not infer or omit these
keys; use the per-action dataset table above. For the full SDK internals, read
`references/sdk-orchestration.md`.

## Spec Param / Parent Model Inference

For SDK-backed DINO jobs that infer checkpoint, output, or encryption fields
from parent jobs, read `references/inference-mappings.md`. Keep generated
runners pointed at that reference instead of patching `config.json` or guessing
checkpoint paths.
